...
void CssParser::parseRule(const std::string& selector, const std::string& properties) {
  ...
  while (start < len) {
    ...
    if (!singleSelector.empty()) {
      ...
      if (style.hasTextAlign || style.hasFontStyle || style.hasFontWeight || style.hasDirection || style.hasDisplay ||
          style.hasTextDecoration) {
...
void CssParser::parseProperty(const std::string& name, const std::string& value, CssStyle& style) {
  if (name == "text-align") {
    ...
    if (v.find("underline") != std::string::npos) {
      style.textDecoration = CssTextDecoration::Underline;
      style.hasTextDecoration = true;
    } else if (v.find("line-through") != std::string::npos) {
      style.textDecoration = CssTextDecoration::LineThrough;
      style.hasTextDecoration = true;
    } else if (v == "none") {
